...
const Home = () => {
  ...
  const handleAddTask = (newTask) => {
    // Mock adding a task (later replace with API call)
    const task = {
      ...newTask,
  ...
  const handleUpdateTask = (taskId, updatedTask) => {
    // Mock updating a task (later replace with API call)
    setTasks(tasks.map(task => (task.task_id === taskId ? { ...task, ...updatedTask } : task)));
  };

  const handleDeleteTask = (taskId) => {
    // Mock deleting a task (later replace with API call)
    setTasks(tasks.filter(task => task.task_id !== taskId));
  };

  const handleLogout = () => {
    // Clear JWT cookie by calling logout endpoint (mock for now)
